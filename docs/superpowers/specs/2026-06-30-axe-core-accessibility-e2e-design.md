...
3ページへ検査を広げた時点で、ダークテーマのAboutと代表記事に `serious` の `color-contrast` 違反を検出した。対象はAboutの関心分野3件と、代表記事のタグ・公開日である。

両方ともライトテーマ用の `text-neutral-500` (`#737373`) を明示する一方でダークテーマ用文字色がなく、`dark:bg-neutral-950` (`#0a0a0a`) 上でも同じ色を使っていた。コントラスト比は `4.17:1` で、通常テキストに必要な `4.5:1` を下回る。

修正は `src/pages/about.astro` と `src/components/PostMetadata.astro` の対象コンテナへ `dark:text-neutral-400` を追加する。同じ `.dot-separated` メタデータを表示する `PostItem.astro` の既存パターンと統一でき、ライト表示を変えず、ダークテーマで `7.66:1` を確保できる。`exclude()` や `disableRules()` で違反を隠す対応は行わない。
