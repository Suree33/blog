...
- 現行記事で使っている非折りたたみの `INFO`、`NOTE`、`WARNING` を対象とする
- 未対応のCallout記法は通常のblockquoteとして残す
- GitHub風テーマCSSも `src/styles/` でローカル所有する
- 復元後に `rehype-callouts` 依存を削除する
- Astro公式の `getViteConfig()` でVitestを導入し、Markdown変換を直接テストする
